...
RSpec.feature "Admins can manage a user's roles" do
  ...
  let!(:ie) { FactoryGirl.create(:project, name: "Internet Explodo") }
  ...
  scenario "when assigning roles to an existing user" do
    ...
    select "Viewer", from: "Internet Explodo"
    ...
    expect(page).to have_content "Internet Explodo: Viewer"
  ...
  scenario "when assigning roles to a new user" do
    ...
    select "Editor", from: "Internet Explodo"
    click_button "Create User"

    click_link "newuser@ticketee.com"
    expect(page).to have_content "Internet Explodo: Editor"
